...
<div class="x_car_book_sider_main_Wrapper my-4">
    <div class="container">
        <form id="estimateForm" name="estimateForm" method="post" class="form-validate-jquery">
            ...
            <input type="hidden" name="subSeq" id="subSeq" value="${sSeq}">
            <div class="row">
                <div class="col-xl-9">
                    <div class="card">
                        <div class="card-header bg-white">
                            <h4 class="mb-0">견적 수정</h4>
                        </div>
                        <div class="card-body">
                            <div class="form-group col-12">
                                <label for="selectCar">렌트카</label>
                                <select name="selectCar" id="selectCar" class="select" onchange="selectCarFunc()" required>
                                    <option disabled>렌트카를 선택하세요. 선택된 렌트카는 견적 제출 이후 예약불가 상태로 변경됩니다.</option>
                                    <optgroup label="예약 가능 렌트카">
                                        <c:forEach var="car" items="${carList}">
                                            <option value="${car.seq}" <c:if test="${car.seq == submit.carVO.seq}"> selected</c:if>>
                                                    ${car.brandCodeVO.name} ${car.modelCodeVO.name} ${car.trimCodeVO.name} / ${car.segmentCodeVO.name} / ${car.carNum}
                                            </option>
                                        </c:forEach>
                                    </optgroup>
                                </select>
                            ...
                            <ul class="list-unstyled row">
                                            ...
                                            <div class="mb-2 col-lg-3 col-md-4 col-6">
                                                <div class="custom-control custom-checkbox custom-control-inline">
                                                    <input type="checkbox"
                                                           ...
                                                           value="${option.optCode}" <c:if test="${submit.itemsList.contains(option.optCode)}">selected</c:if> >
                                ...
                                <li class="col-md-12 mb-3 ">
                                    ...
                                    <div class="border">
                                        <textarea name="memo" id="memo" class="form-control border-bottom-0 px-2">${submit.memo}</textarea>
                                    </div>
                                </li>
                                <li class="col-md-12 mb-3 ">
                                    <label class="h5" for="price">렌트 가격</label>
                                    <div class="form-group">
                                        <input type="text" name="price" id="price" class="form-control number-only" value="${submit.price}">
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="card-footer">
                            <button type="submit" class="btn btn-lg btn-primary w-100">수정 완료</button>
                        </div>
                    </div>
                </div>
                <div class="col-xl-3">
                    <div class="card">
                        <c:set var="est" value="${submit.estimateHistoryVO}" />
                        <div class="card-header bg-white">
                        ...
                        <div class="card-body">
                            <div class="x_car_access_filer_top_img">
                                ...
                                <h5 class="pt-2">${est.brandCodeVO.name} ${est.modelCodeVO.name}<br>
