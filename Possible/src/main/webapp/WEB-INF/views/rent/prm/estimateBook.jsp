...
<c:set var="during" value="${estimate.endDate.date - estimate.startDate.date}"/>
            ...
            <div class="container py-2">
                <div class="page-header-content header-elements-inline">
                    <div class="page-title text-white">
                        <h1 class="font-weight-bold">프리미엄 렌트 예약</h1>
                ...
                <div class="x_car_book_left_siderbar_wrapper float_left">
                    ...
                    <div class="car-filter accordion x_inner_car_acc_accor card">
                        ...
                        <div class="card-img p-3 border-bottom">
                            <img class="card-img-top img-fluid" src="${pageContext.request.contextPath}${submit.carVO.modelCodeVO.img}" alt="car_img">
                            <h6 class="font-weight-bold mt-2 mb-1">${submit.carVO.brandCodeVO.name} ${submit.carVO.modelCodeVO.name}</h6>
                            <p class="mb-0">${submit.carVO.trimCodeVO.name}</p>
                            <hr>
                            <p>대여가격<span class="h5 float-right text-primary font-weight-bold"><fmt:formatNumber value="${submit.price * during}" pattern="#,###"/>원</span></p><div class="clearfix"></div>
                            <p>보험가격<span class="h5 float-right text-primary font-weight-bold">수령시 가입</span></p><div class="clearfix"></div>
                        </div>
                        <div class="card-body">
                            <div class="form-group position-relative">
                                <h6 class="font-weight-bold">대여 날짜</h6>
                                <p>
                                    <fmt:formatDate value="${estimate.startDate}" pattern="yyyy년 MM월 dd일 (EEE)" />
                                </p>
                            </div>
                            <div class="form-group position-relative">
                                <h6 class="font-weight-bold">반납 날짜</h6>
                                <p>
                                    <fmt:formatDate value="${estimate.endDate}" pattern="yyyy년 MM월 dd일 (EEE)" />
                                </p>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <ul class="list-unstyled">
                                    ...
                                    <li><i class="icon-check2 pr-2"></i>대여업체 : ${submit.companyVO.name}</li>
                                    <li>
                                        <i class="icon-check2 pr-2"></i>차량정보
                                        <ul>
                                            <li>연식 : ${submit.carVO.year}년식</li>
                                            <li>연료 : ${submit.carVO.fuelCodeVO.name}</li>
                                            <li>색상 : ${submit.carVO.colorCodeVO.name}</li>
                                            <li>기어 : ${submit.carVO.missionCodeVO.name}</li>
                                            <li>연비 : ${submit.carVO.kmpl}km/l</li>
                                        </ul>
                                    </li>
                        ...
                        <div class="x_car_acc_filter_bottom_total card-footer text-center">
                            <h5>총 결제금액 <span class="h3 text-primary font-weight-bold pl-2"><fmt:formatNumber value="${(submit.price * during)}" pattern="#,###"/>원</span></h5>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="x_carbooking_right_section_wrapper card">
                    ...
                    <div class="card-body">
                        <form id="payFrm" name="payFrm" class="billing-form">
                            <input type="hidden" name="rentType" value="RTP02">
                            <input type="hidden" name="start" value="<fmt:formatDate value="${estimate.startDate}" pattern="yy/MM/dd" />">
                            <input type="hidden" name="end" value="<fmt:formatDate value="${estimate.endDate}" pattern="yy/MM/dd" />">
                            <input type="hidden" name="price" value="${(submit.price) * during}">
                            <input type="hidden" name="cmpn_postal" value="${submit.companyVO.postal}">
                            <input type="hidden" name="cmpn_addr1" value="${submit.companyVO.addr1}">
                            <input type="hidden" name="cmpn_addr2" value="${submit.companyVO.addr2}">
                            <input type="hidden" name="submit" value="${submit.seq}">
                            <input type="hidden" name="estimate" value="${estimate.seq}">
                            <input type="hidden" name="car" value="${submit.carVO.seq}">
                            <input type="hidden" name="user" value="${estimate.memberVO.seq}">
                            <input type="hidden" name="cmpn" value="${submit.companyVO.seq}">
                            <div class="row">
                                <div class="col-12">
...
<script>
    ...
    $(document).ready(function(){
        ...
        function paymentFnc() {
            ...
            let estimate = payFrm.estimate.value;

            IMP.request_pay({
                pg : 'html5_inicis',
                pay_method : payMethod,
                merchant_uid : merchantUid,
                name : '${submit.carVO.brandCodeVO.name.concat(" ").concat(submit.carVO.modelCodeVO.name)} 여행갈카', /* 주문명 */
                amount : price, /* 가격 */
                buyer_name : receiver,
                ...
                if (rsp.success) {
                    // DB에 데이터 삽입
                    $.ajax({
                        url: '${pageContext.request.contextPath}/premiumRent/estimate/payment',
                        type: 'POST',
                        dataType: 'text',
