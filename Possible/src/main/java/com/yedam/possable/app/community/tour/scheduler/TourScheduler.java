package com.yedam.possable.app.community.tour.scheduler;
...
public class TourScheduler {
	...
	public void auto() throws IOException {
		...
		urlBuilder.append("&" + URLEncoder.encode("contentTypeId", "UTF-8") + "="
				+ URLEncoder.encode("12", "UTF-8")); /* 관광타입 12=관광지 14=문화시설 */
		...
		// String -> JSON 변환
		JSONArray jArry = new JSONObject(result).getJSONObject("response").getJSONObject("body").getJSONObject("items")
				.getJSONArray("item");
...
//				new JSONObject(new JSONObject(sb.toString()).get("body").toString()).getJSONArray("items").toString()).getJSONArray("item");
		...
		for (int i = 0; i < jArry.length(); i++) {
			...
			if (JO.getLong("modifiedtime") > updateDate) {
				System.out.println(test.getTitle());
				if (contentIdList.contains(JO.getLong("contentid"))) {
					// update
